...
def process_image_inference(image):
    ...
    model = torch.hub.load(r"C:\Users\vrush\Documents\Coding\Projects\yolov5", 'custom', path=r"C:\Users\vrush\Documents\Coding\Projects\yolov5\best.pt", source='local') 
    # Image
    img = image
    # Inference
    results = model(img)
...
# Criteria to filter bounding boxes
    ...
    for index, row in df.iterrows():
        if row['confidence'] > 0.7 and is_non_squarish(row['xmin'], row['ymin'], row['xmax'], row['ymax']):
            ...
            bounding_boxes[len(bounding_boxes)] = {'xmin': xmin, 'ymin': ymin, 'xmax': xmax, 'ymax': ymax}
